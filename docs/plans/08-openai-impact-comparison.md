...
## 재비교 3 — gpt-5.4 풀 모델 + effort medium (2026-07-07, 동일 06-26·150클러스터)

"오프셋이 mini 한정인지 GPT-5.4 계열 성향인지" 확인차 v3와 동일 설정에서 모델만
`gpt-5.4`($2.50/$15 per 1M — mini의 3.3배)로 교체. 코드 변경 없음(`--model` 플래그).
덤프는 `out_compare_v4/`. 스냅샷 cost_usd는 mini 단가 기준이라 아래 표는 실단가로 환산.

| 지표 | openai v3 (mini·medium) | openai v4 (gpt-5.4·medium) | anthropic |
|---|---|---|---|
| ok / empty / degraded | 150 / 240 / 0 | 150 / 240 / 0 | 150 / 240 / 0 |
| 인용 총수 / ok당 | 393 / 2.62 | 450 / 3.0 | 538 / 3.59 |
| impact 평균 / 중앙값 | 54.4 / 58 | 54.2 / 58 | 36.4 / 35 |
| confidence | HIGH 34 · MED 111 · LOW 5 | HIGH 23 · MED 115 · LOW 12 | HIGH 7 · MED 65 · LOW 78 |
| 쌍별 diff 평균 vs anthropic¹ | +18.0 | +17.8 | — |
| reasoning 토큰/콜 | ~796 | ~646 | — |
| 소요 / drop율 / 비용 | 1,150s / 1.0% / $0.77 | 1,630s / **0.0%** / $2.36 | 2,807s / — / — |

¹ 재비교 2와 동일한 항등식 유도(세 실행 모두 ok = 동일한 분석 150클러스터).

소견:

- **점수 오프셋은 풀 모델에서도 동일(+17.8)**: 평균 54.2·중앙값 58이 mini와 사실상 같다.
  HIGH는 34→23으로 줄고 LOW 5→12로 소폭 회복했지만 anthropic(7/65/78)과는 여전히
  다른 분포. 오프셋은 mini 한정이 아니라 **GPT-5.4 계열의 성향**.
- **인용 품질은 풀 모델이 뚜렷이 우위**: drop율 0.0%(환각 인용 0건, mini 1.0%),
  인용 ok당 3.0(mini 2.62)으로 anthropic(3.59, 중복 포함)에 근접.
- **추적 케이스**: 1256(은행 ESG 스텁) 42/MED — mini v3(38/MED)와 비슷, anthropic
  25/LOW보단 높음. 1009(링크팜 스텁) 38/LOW/중립 — 유일하게 "혼재 클러스터"임을
  분석문에 명시하고 LOW를 부여한 실행(가장 정확한 처리).
- **비용**: $2.36/일 — mini($0.25~0.77)의 3~9배. 속도 10.9s/클러스터(anthropic 18.7s의
  1.7배 빠름, mini 7.7s보다 느림).

**최종 결론**: 점수 캘리브레이션 격차(+16~18)는 프롬프트 보강(v2)·reasoning(v3)·모델
업그레이드(v4) 모두로 해소되지 않았다 — **GPT-5.4 계열 자체의 점수 스케일**로 확정.
교체의 선행 조건은 변함없이 사후 리캘리브레이션 또는 랭킹 임계값 재베이스라인이다.
품질(인용 무결성·꼬리 케이스)은 gpt-5.4 > gpt-5.4-mini > (v1) 순으로 개선되므로,
리캘리브레이션을 진행하게 되면 후보는 gpt-5.4(비용 허용 시) 또는 mini+medium.
