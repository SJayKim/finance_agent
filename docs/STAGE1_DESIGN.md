...
| 임베딩 | `[DECIDED]` **bge-m3 / dim 1024**(STAGE1.5 §6), 인터페이스는 추상화 | `config.embedding_model="BAAI/bge-m3"`, `embedding_dim=1024`(`models.Vector(1024)`와 고정 일치). 로컬(sentence-transformers) ↔ API 교체 가능 |
...
- 쿼리: 커버리지 종목/섹터 키워드 + 별칭 사전. `sort=date`.
  - **구현:** 하드코딩 없이 `naver.load_coverage_queries(session)`가 coverage 섹터(non-null)
    ∪ 커버 종목의 `security_aliases` 별칭에서 도출(§2; EDGAR `ciks=[]`와 대칭). 빈 DB → 빈
    쿼리 → 네이버 no-op. `runner.build_default_connectors`가 세션을 열어 주입한다.
...
> **구현 현황:** `app/pipeline/coingecko.py`는 시세/트렌드 raw 수집기가 아니라 **CRYPTO
> 별칭 시더**다 — `/coins/list`에서 `_UNIVERSE` 화이트리스트 id만 골라 `security_aliases`
> (name→SYMBOL/CRYPTO)에 적재(§6.4). 아래 시세·trending·categories raw 수집은 **미구현/이연**.
...
### 5.5 KRX (KR 시세 EOD) `[PLANNED: 커넥터 미구현]`
- OpenAPI로 일별 종가/거래량. 실시간 아님(Stage 1).
- **구현 현황:** KRX 커넥터는 아직 없다. 일일 수집(`runner.build_default_connectors`)은
  6소스(rss·naver·opendart_docs·edgar_docs·marketaux·finnhub)뿐 — KRX 시세는 이연.
...
> **구현 현황:** 독립 분류 단계는 없다. `event_type`+`direction`+`confidence`는 **별도
> 분류기가 아니라 §7 `analyze_impact`의 2-패스 패스2에서** 함께 산출된다
> (`pipeline.py` analyze_impact가 brief_item에 채움). `pipeline.event_classify()`는
> `NotImplementedError` placeholder다. 따라서 실행 순서는 dedup → cluster →
> generate_impact(골격) → analyze_impact(§7) → ticker_link → embed
> (`run_pipeline` docstring 기준).
...
raw_documents(id, source_id, external_id UNIQUE(source_id,external_id),
              ...
              raw_payload jsonb, embedding Vector(1024))  -- HNSW vector_cosine_ops 인덱스
clusters(id, brief_date, centroid Vector(1024), representative_doc_id, created_at)
...
coverage(id, analyst_id, ticker|sector, market)      -- §9
security_aliases(alias, ticker, market) PK(alias,ticker,market)  -- §6.4 티커 링킹 사전(coingecko 시더)
audit_log(id, ts, actor, action, payload jsonb)      -- 컴플라이언스 추적
daily_digests(id, brief_date, section, heading, body_text,        -- STAGE1.5 §7
              status[ok|degraded|empty], generated_at) UNIQUE(brief_date,section)
digest_sources(digest_id, brief_item_id)             -- 다이제스트 섹션 ← brief_item 출처
```
...
### 11.3 ~~`[OPEN]` 한국어 임베딩 모델 선택~~ → `[DECIDED]` (STAGE1.5 §6)
**bge-m3 / dim 1024**로 확정(`config.embedding_model="BAAI/bge-m3"`, `embedding_dim=1024`;
`models.Vector(1024)`와 고정 일치). 다국어 로컬 모델. 차원 변경 시 재임베딩+마이그레이션 필요.
