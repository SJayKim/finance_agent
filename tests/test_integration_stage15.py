...
# run_daily의 _count_embedded는 fetched_at(서버 now)를 brief_date(KST) 윈도우로 거른다.
# 가짜 커넥터가 "지금" 적재하므로 brief_date도 KST 오늘이어야 한다 — 고정 과거일이면 fetched_at이
# 윈도우 밖이라 embedded=0으로 그날 이후 매일 실패하는 날짜 결합 회귀가 된다.
