...
- 운영 LLM 부하를 올리는 변경(모델 상향·reasoning_effort=medium 등)을 daily 파이프라인에 넣기 전에 `gh run list --workflow=daily.yml`로 최근 실행시간을 확인해 timeout cap(현 90분) 대비 여유를 먼저 검증할 것. gpt-5.4 전환만으로 이미 90분 cap을 넘겨 취소·브리프 손실 상태였는데, 이를 모른 채 impact effort=medium(콜당 ~3x)을 얹어 부하를 가중했다. build_digest가 run_pipeline 뒤라 초과 시 그날 브리프가 통째로 미생성된다. (2026-07-08, .github/workflows/daily.yml timeout-minutes / app/llm/factory.py)
- 외부 LLM/HTTP 콜엔 per-call timeout이 필수. litellm은 `timeout` 미지정 시 스톨된 콜을 무한 블록한다(실측 2026-07-08: digest 콜 57분 hang → GitHub 90분 guillotine 취소 → build_digest 미완 → 그날 브리프 손실. 위 timeout-cap 초과의 실제 원인은 만성 지연이 아니라 이 hang이었다). gateway 전송 계층에 `_REQUEST_TIMEOUT_S`(현 120s) 고정 — 타임아웃은 LLMError로 정규화돼 graceful degrade로 떨어진다. 커넥터는 httpx `timeout=` 명시(수집·openfigi·coingecko·sec·opendart 모두 10~30s 있음). (2026-07-08, app/llm/gateway.py openai_responses/anthropic_messages)
...
4. After fixing, ask: "Would a rule in this AGENTS.md have prevented
   this error?"
